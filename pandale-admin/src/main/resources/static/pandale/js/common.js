...
var refreshItem = function(){
	...
	var target = $('.pandale_iframe[data-id="' + currentId + '"]', topWindow);
...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', topWindow).each(function() {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                // 显示tab对应的内容区
                $('.mainContent .pandale_iframe', topWindow).each(function() {
                    if ($(this).data('id') == dataUrl) {
    ...
    if (flag) {
        ...
        $('.mainContent', topWindow).find('iframe.pandale_iframe').hide().parents('.mainContent').append(str1);
